...
# PM 2.5
data = pd.read_csv("pmdatos\encinaDia_25.csv", sep = ";", encoding = "ISO=8859-1", decimal = ",")
...
# PM 10
data = pd.read_csv("pmdatos\encinaDia_10.csv", sep = ";", encoding = "ISO=8859-1", decimal = ",")
result = data.values.tolist()
pm10 = []
...
# TEMPERATURA
data = pd.read_csv("pmdatos\Encina_Tempdia.csv", encoding = "ISO=8859-1")
result = data.values.tolist()
...
for a in result:
    c = -276 # Nunca se va llegar a esta T xD
    if pd.notna(a):
        c = a[0]
...
for i in range(0,n):
    ...
    if pm25[i] != -1 and temperatura[i] != -276:
...
b1 = estimate_coef(dataPM10, temp)
plot_regression_line(dataPM10, temp, b1)
