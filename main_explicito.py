...
####################### CONDICIONES DE BORDE TRANSMISIVAS
...
@jit
def adveccion(G_f, u_N, u_O):
    adv = ((G_f + np.abs(G_f))/2 * u_O) + ((G_f - np.abs(G_f))/2 * u_N)
    return adv

@jit
def difusion(u_O, u_N, nEdge, tf_lf, delta_dist):
    diff = coeff_D * (((u_N - u_O) / delta_dist) - (tf_lf / delta_dist)) * aLongLados[nEdge]
    return diff
...
def f_semidiscreto(nCell, uPM, aUPM_vtc, t):
    ...
    for i in range(3):
        ...
        if nCell_v == -1:
            ...
            u_N = uPM[nCell_v]
            #flujo difusivo
            diff_f = difusion(u_O, u_N, nEdge, t_f, delta_dis)
...
export = dfPM10.to_csv(r'visual/PM_estimadoExpT.csv', index = None, header=True)

dfExac = pd.DataFrame(hExacta)
export = dfExac.to_csv(r'visual/PM_exactoExpT.csv', index = None, header=True)
